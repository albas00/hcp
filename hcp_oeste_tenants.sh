...
#printf "%s\n" "==      HCP TENANTS  OESTE            ==" >> $DIR"hcp_oeste_tenants.txt"
#printf "%s\n" "========================================" >> $DIR"hcp_oeste_tenants.txt"
#printf "%s\n" " " >> $DIR"hcp_oeste_tenants.txt"

#printf "%-14s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s\n" "FECHA-HORA" "BOADILLA" "BOA_NEW" "CTERA" "EV_CAN" "HDI01" "HDI02" "HDI03" "HDI04" "HDI05" "HDI06" "HDI07" "HDI08" >> $DIR"hcp_oeste_tenants.txt"
...
hdi8=`curl -X GET -k -H "Authorization: HCP ${TOKEN}" -H "Accept: application/json" "https://admin.hdi.bosthcp.cloud.corp:9090/mapi/tenants/vmnxbhdistp08/statistics?prettyprint" |grep "storageCapacityUsed" |awk -F, '{print $1}' |awk '{print $3}'`
