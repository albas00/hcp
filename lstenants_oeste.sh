...
#printf "%-10s %-5s %-20s %-15s %-15s %s\n" "Dia" "Hora" "Name" "Quota (GB)" "Used (GB)" "Available (GB)" > $DIR"hcp_consumo.txt"
		...
		for HDI in ${HDIS} ; do
  			...
  			printf "%-10s %-5s %-20s %-15s %-15s %s\n" "$DIA" "$HORA" "$HDI"_OESTE "$HARDQUOTA" "$USEDSPACE" "$AVAILABLE" >> $DIR"hcp_oeste_consumo.txt" 
			echo $DIA" "$HORA","$HDI"_OESTE,"$HARDQUOTA","$USEDSPACE >> $DIR"hcp_oeste_consumo.csv"
